...
def can_handle_current_context(context, event):
    ...
    if (event.Message is None) or (event.Message.Text is None):
        return False
    elif "lantra" in event.Message.Text.lower():
        return True
    else:
        return False
...
def handle(conversation, event, args):
    ...
    photo = InputOnlineFile("https://raw.githubusercontent.com/elementh/foscbot/main/assets/lantra.jpg")
    Navigator.SendPhotoAsync(args.client, args.chat, photo, caption="Elantro")
