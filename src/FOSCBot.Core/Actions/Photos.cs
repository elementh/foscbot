...
public static class Photos
{
    ...
    {
        catalog.OnSticker((Sticker sticker) =>
        {
            if (RandomProvider.GetThreadRandom()!.NextDouble() > 0.5d) return false;

            return sticker.SetName?.Equals("foscupct") is not false && sticker.Emoji?.Equals("😚") is true;
